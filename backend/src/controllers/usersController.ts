...
export const createNewUser: RequestHandler = asyncHandler(async (req, res, next) => {
    ...
    const userEmailExist = await User.findOne({ email }).collation({locale: 'en', strength: 2}).exec()
    if (userEmailExist) {
        ...
        (err, emailToken) => {
            try {
                transporter.sendMail({
                    ...mailOptions(email, `http://localhost:3000/confirmation/${emailToken}`),
                })
            } catch (err) {
                // dac res blad
                console.log(err);
...
export const confirmUser: RequestHandler = asyncHandler(async (req, res, next) => {
...
export const resendConfirmationEmail: RequestHandler = asyncHandler(async (req, res, next) => {
