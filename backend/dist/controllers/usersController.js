...
exports.createNewUser = (0, express_async_handler_1.default)((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const { username, email, password } = req.body;
    if (!username || !password || !email) {
        res.status(400).json({ message: 'Credentials are required!' });
        return;
    }
    const userExist = yield User_1.default.findOne({ username }).collation({ locale: 'en', strength: 2 }).exec();
    if (userExist) {
        res.status(408).json({ message: 'User already exists!' });
        return;
    }
    const userEmailExist = yield User_1.default.findOne({ email }).collation({ locale: 'en', strength: 2 }).exec();
    if (userEmailExist) {
        res.status(408).json({ message: 'Your email already registered. Try to log in!' });
        return;
    }
    ...
    jsonwebtoken_1.default.sign({
        ...
        try {
            nodemailer_1.transporter.sendMail(Object.assign({}, (0, nodemailer_1.mailOptions)(email, `http://localhost:3000/confirmation/${emailToken}`)));
        }
        catch (err) {
            // dac res blad
            console.log(err);
    ...
    if (user) {
        res.status(201).json({ message: `New user ${username} created. Confirm your account via email.` });
    }
    else {
        res.status(400).json({ message: 'Invalid user data received.' });
    }
}));
exports.confirmUser = (0, express_async_handler_1.default)((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    jsonwebtoken_1.default.verify(emailToken, process.env.EMAIL_SECRET, (err, decoded) => __awaiter(void 0, void 0, void 0, function* () {
        if (err) {
            ...
            return res.status(400).json({ message: 'Unexpected error occured. Try again.' });
        }
        ...
        if (userFound === null || userFound === void 0 ? void 0 : userFound.confirmed) {
            return res.status(200).json({ message: 'User is already confirmed. Please log in.' });
        }
        yield User_1.default.updateOne({ _id: userId }, { $set: { confirmed: true } });
        res.status(200).json({ message: 'User confirmed. Please log in.' });
    }));
}));
exports.resendConfirmationEmail = (0, express_async_handler_1.default)((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!user) {
        res.status(408).json({ message: 'Your email is not registered. Sign up!' });
        return;
    }
    jsonwebtoken_1.default.sign({
        "UserInfo": {
